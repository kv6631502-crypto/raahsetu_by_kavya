...
export default function ConstellationGrid({
    title = "Constellation",
    subtitle = "High-velocity dynamic mesh. Sweep your cursor quickly across the grid to unleash kinetic shockwaves.",
    className = "",
    children,
    fullHeight = true,
    ...
    // Sync theme preference (supports system and explicit .dark / .light classes)
    useEffect(() => {
        const checkTheme = () => {
    ...
    return (
        <div
            ref={containerRef}
            className={`relative w-full overflow-hidden select-none bg-slate-950 dark:bg-slate-950 light:bg-slate-50 ${
                fullHeight ? "h-screen min-h-[550px]" : "h-full min-h-[400px]"
            } ${className}`}
        >
            <canvas ref={canvasRef} className="absolute inset-0 block cursor-crosshair" />

            {/* Seamless overlay title or custom children */}
            {children ? (
                <div className="relative z-10 flex h-full flex-col items-center justify-center text-center px-4 pointer-events-auto">
                    {children}
                </div>
            ) : (
